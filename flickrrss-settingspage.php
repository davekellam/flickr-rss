	...
	<form method="post">
		<table class="form-table">
			<tr valign="top">
				...
				<td>
					<select name="flickrRSS_num_items" id="flickrRSS_num_items">
						<?php for ( $i = 1; $i <= 20; $i ++ ) { ?>
							<option <?php if ( $settings['num_items'] == $i ) {
								echo 'selected';
							} ?> value="<?php echo $i; ?>"><?php echo $i; ?></option>
						<?php } ?>
					</select>
					<select name="flickrRSS_type" id="flickrRSS_type">
						<option <?php if ( $settings['type'] == 'user' ) {
							echo 'selected';
						} ?> value="user">user
						</option>
						<option <?php if ( $settings['type'] == 'set' ) {
							echo 'selected';
						} ?> value="set">set
						</option>
						<option <?php if ( $settings['type'] == 'favorite' ) {
							echo 'selected';
						} ?> value="favorite">favorite
						</option>
						<option <?php if ( $settings['type'] == 'group' ) {
							echo 'selected';
						} ?> value="group">group
						</option>
						<option <?php if ( $settings['type'] == 'public' ) {
							echo 'selected';
			...
			<tr valign="top" id="userid">
				...
				<td>
					<input name="flickrRSS_id" type="text" id="flickrRSS_id" value="<?php echo $settings['id']; ?>" size="20" />
					...
					<table style="margin-left: -10px">
						...
						<tr>
							<td valign="top" style="border-width: 0px;">
								<label for="flickrRSS_html">Item HTML:</label><br />
								<textarea name="flickrRSS_html" type="text" id="flickrRSS_html" style="width:400px;" rows="10"><?php echo htmlspecialchars( stripslashes( $settings['html'] ) ); ?></textarea>
							</td>
							<td valign="top" style="border-width: 0px;">
								<div>
						...
						<tr>
							<td valign="top" colspan="2" style="border-width: 0px;">
								<label for="flickrRSS_after_list">After List:</label><br />
								<input name="flickrRSS_after_list" type="text" id="flickrRSS_after_list" value="<?php echo htmlspecialchars( stripslashes( $settings['after_list'] ) ); ?>" style="width:400px;" />
							</td>
						</tr>
					</table>
				</td>
		...
		<p>This allows you to store the images on your server and reduce the load on Flickr. Make sure the plugin works without the cache enabled first. If you're still having
			trouble, try visiting the <a href="http://groups.google.com/group/flickrrss/">flickrRSS forum</a> for help.
		</p>
		<table class="form-table">
			<tr valign="top">
				<th scope="row" colspan="2" class="th-full">
					<input name="flickrRSS_do_cache" type="checkbox" id="flickrRSS_do_cache" value="true" <?php if ( $settings['do_cache'] == 'true' ) {
			...
			<tr valign="top" class="cachesettings">
				...
				<td>
					...
					foreach ( array( "small", "square", "thumbnail", "medium", "large" ) as $size ) {
						...
						if ( is_array( $settings['cache_sizes'] ) && in_array( $size, $settings['cache_sizes'] ) ) {
							echo ' checked="checked" ';
						}
			...
			<tr valign="top" class="cachesettings">
				<th scope="row">URL</th>
				<td>
					<input name="flickrRSS_cache_uri" type="text" id="flickrRSS_cache_uri" value="<?php echo $settings['cache_uri']; ?>" size="50" /><br />
					Could be <code><?php echo trailingslashit( get_option( 'siteurl' ) ); ?>wp-content/cache/</code> ?
				</td>
			</tr>
			<tr valign="top" class="cachesettings">
				<th scope="row">Full Path</th>
				<td>
					<input name="flickrRSS_cache_path" type="text" id="flickrRSS_cache_path" value="<?php echo $settings['cache_path']; ?>" size="50" /><br />
					Could be <code><?php echo trailingslashit( realpath( "../wp-content/cache" ) ); ?></code> ?
				</td>
			</tr>
		</table>
		<div class="submit">
